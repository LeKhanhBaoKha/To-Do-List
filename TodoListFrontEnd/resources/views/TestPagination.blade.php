...
@endphp
    <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
        ...
        @if ($currentPage > 1)
            ...
            @elseif(strpos($todosWithPage['path'], 'projects'))
            <a href="projectIndexPage?http://localhost:8008/api/serve/projects?page={{$currentPage -1}}" aria-current="page" class="relative inline-flex items-center px-4 py-2 text-sm font-semibold text-gray-900 ring-1 ring-inset ring-gray-300 hover:bg-purple-100 hover:text-black focus:z-20 focus:outline-offset-0 transition duration-300 ease-linear rounded-l-lg"><x-fas-angle-left class="w-[10px]"/></a>
            @else
        ...
        @for ($i =$startPage; $i <= $endPage; $i++)
            ...
            @elseif(strpos($todosWithPage['path'], 'projects'))
            <a href="projectIndexPage?http://localhost:8008/api/serve/projects?page={{$i}}" aria-current="page" class="relative inline-flex items-center px-4 py-2 text-sm font-semibold text-gray-900 ring-1 ring-inset ring-gray-300 hover:bg-purple-100 hover:text-black focus:z-20 focus:outline-offset-0 transition duration-300 ease-linear {{$i == $currentPage ? 'bg-purple-500 text-white': '' }}">{{$i}}</a>
            @else
